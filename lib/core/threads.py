...
from lib.core.exception import GeccoConnectionException
from lib.core.exception import GeccoUserQuitException
from lib.core.exception import GeccoValueException
...
def run_threads(num_threads, thread_function, args: tuple = (), forward_exception=True, start_msg=True):
    ...
    except (KeyboardInterrupt, GeccoUserQuitException):
        kb.thread_continue = False
        kb.thread_exception = True
    ...
    except (GeccoConnectionException, GeccoValueException) as ex:
        kb.thread_exception = True
        logger.error("thread {0}: {1}".format(threading.currentThread().getName(), str(ex)))
        if conf.verbose > 1:
            traceback.print_exc()
