...
from lib.core.enums import OS_ARCH, OS
...
from lib.core.settings import IPV6_ADDRESS_REGEX
...
def inject_to_data(data:dict, payload:str, is_override=True):
    ret_data_list = []
    if type(data) == dict and type(payload) == str:
        for dk, dv in data.items():
            bak_data = copy.deepcopy(data)
            if is_override:
                bak_data[dk] = payload
            else:
                bak_data[dk] = dv + payload
            ret_data_list.append(bak_data)
...
def format_cookies_to_playwright(cookie, url):
    cookies = []
    if type(cookie) == str:
        dict_cookies = extract_cookies(cookie)
    elif type(cookie) == dict:
        dict_cookies = cookie
    else:
        dict_cookies = {}
    for ck_key, ck_value in dict_cookies.items():
        cookies.append({'name': ck_key, 'value': ck_value, "url": url})
    return cookies

def extract_cookies(cookie):
    return dict([i.strip().split("=", 1) for i in cookie.split(";")])

def compress_cookies(cookie):
    return "; ".join([f"{k}={v}" for k, v in cookie.items()])


def params_to_dict(data):
    if type(data) == str:
        if data.startswith("{"):
            data = json.loads(data)
        else:
            if "=" in data:
                data = dict(i.split("=", 1) for i in data.split("&"))
            else:
                data = {}
    elif type(data) == dict:
        pass
...
def write_file(data, file_ext='', file_name=''):
    ...
    if not file_name:
        file_name = TIMESTAMP
    file_name += file_ext
    file_path = os.path.join(paths.GECCO_TMP_PATH, file_name)
...
def recycle_get_pyfile(dir, pyfiles):
    for root, _, files in os.walk(dir):
        files = filter(lambda x: not x.startswith("__") and x.endswith(".py"), files)
        pyfiles.extend(map(lambda x: os.path.join(root, x), files))
        for sub_dir in _:
            recycle_get_pyfile(sub_dir, pyfiles)
